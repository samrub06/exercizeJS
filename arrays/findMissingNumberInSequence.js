...
function findMissingNumberInSequenceV2(numbers) {
  const n = numbers.length;
  const sumExpected = (n * (n + 1)) / 2;
  const sumNumber = numbers.reduce((acc, current) => acc + current, 0);
  return sumExpected - sumNumber;
}

const numbers = [1, 3, 0];

findMissingNumberInSequenceV2(numbers);
